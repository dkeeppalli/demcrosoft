$.Controller.extend("CCL.Controllers.MostDownloadedTopics", {
    init: function () {
        this.sAppInstallDir = g_sAppInstallDir;
        this.topicListHolder = this.element.find('.topicListHolder');
        CCL.Models.Topic.getTopics({type:"top_downloaded"
									, value:true
									, sortType:"date_added"
									, sortValue:"asc"
									, start:1
	...
	, list: function (statusObj, topiclist) {
		if(statusObj.success){
    		this.topicListHolder.html(this.view( this.sAppInstallDir 
    										  + 'views/topics/mini'
    										  , { topicsList: topiclist, viewMode:"topDownloaded" }));
    	}else{
	...
	, 'a.topicTitle click' : function(){
		CCL.Controllers.PagesHolder.previousBookmark = CCL.Controllers.PagesHolder.makeBookmark("downloaded", ""); 
	}
});