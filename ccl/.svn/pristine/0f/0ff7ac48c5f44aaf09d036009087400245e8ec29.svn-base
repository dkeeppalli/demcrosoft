...
function insertTag(tag) {
            var liEl = '<li id="tag-'+tag+'" class="li_tags">'+
                        ...
                        '<a href="javascript:void(0)" onclick="removeTag(this); return false;"'+
                        ' class="del" id="del_'+tag+'"></a>'+
                        '</li>';
            return liEl;
        }
		$("#"+options.bTag).click(function() {
			$(input).focus();
		});
        ...
        $(input).keydown(function(event) {
            ...
            if (keyCode == 37 && textVal == '') {
                $("#"+options.liTag).insertBefore($("#"+options.liTag).prev());
                $(input).focus();
            }

            // move right (right arrow pressed)
            if (keyCode == 39 && textVal == '') {
                $("#"+options.liTag).insertAfter($("#"+options.liTag).next());
                $(input).focus();
            }

            // delete prev tag (backspace pressed)
            if (keyCode == 8 && textVal == '') {
                deletedTag = $("#"+options.liTag).prev().find(".a_tag").html();
                removeByValue(arrayTags, deletedTag);
                $("#"+options.liTag).prev().remove();
                $(input).focus();
            }

            // delete next tag (delete pressed)
            if (keyCode == 46 && textVal == '') {
                deletedTag = $("#"+options.liTag).next().find(".a_tag").html();
                removeByValue(arrayTags, deletedTag);
                $("#"+options.liTag).next().remove();
                $(input).focus();
            }
            ...
            if ((47 < keyCode && keyCode < 106) || (keyCode == 32)) {
                ...
                if (keyCode != 32) {
                    ...
                    //$("#"+options.liTag).attr("style", "width:"+inputWidth+"px");
                    ...
                    if (isExist == -1) {
                        ...
                        $(insertTag(textVal)).insertBefore("#"+options.liTag);
                   ...
                   // inputWidth = 16;
                    //$(this).attr("style", "width:"+inputWidth+"px");
                   // $("#"+options.liTag).attr("style", "width:23px")
                    $(this).val("");
                } else {
                    $(this).val("");
                }
	...
	var		$input = $(input).attr('autocomplete', 'off');
			$input = $(input).attr('value', 'Type tag(s) Here');
			//$input = $(input).attr('onfocus', 'changevalue(this)');
	...
	function selectItem(li) { /*This is called selecting an item from auto-complete list */
		if (!li) {
		...
		prev = v;
		$results.html(''); //Empties the content loaded in autocomplete div
		$input.val(v);
		 $(insertTag(v)).insertBefore("#"+options.liTag);
